...
- **Gaps** · `/gaps` — identifica lacunas nas três camadas (mecânica: link quebrado/mal formatado; léxica: termo sem página, página sem link; semântica: páginas temáticas próximas sem menção literal), tratando essays, concepts, entities e insights como peers. Read-only — nunca cria página nem insere link. É o primeiro passo interno de `/connect`; ao final, se chamado sozinho, ofereça `/connect` para agir sobre os candidatos.
