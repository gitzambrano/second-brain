...
- **`wiki/`** — espaço de trabalho do LLM, onde tudo é criado e mantido:
  ...
  - `wiki/references.md` e `wiki/references.json` — bibliografia consolidada de todos os essays, agrupada por tipo de fonte. Também artefatos gerados, por `scripts/build_references.py`.
...
| Skill    | Comando       | Quando usar                                                                                                                                                                                                              |
| -------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Sweep    | `/sweep`    | Orquestrar a bateria completa de revisão num essay ou no corpus inteiro:`/organize` (passada mecânica) → `/continuity` → `/proofread` → `/polish` → `/linkify`. Aceita `/sweep` (corpus) ou `/sweep <slug>` (essay único) |
| Organize | `/organize` | Verificar a saúde da base: índice, log, mapa de sources, tags, links, e formatação mecânica de essay (estrutura, byline, LaTeX, aspas, espaçamento, Obsidian-compat — aplica fixes automáticos via `fix_lint.py` e reporta o restante). Aceita `/organize` (corpus inteiro) ou `/organize <slug>` (só formatação/referências/wikilinks daquele essay, pulando as checagens de corpus) |
...
2. Rode `python scripts/build_index.py` sempre que um essay for criado, editado ou removido, para regenerar `wiki/index.md` e `wiki/index.json`. Nunca edite o índice à mão. Se `## Referências` foi tocada, rode também `python scripts/build_references.py`.
...
- **qmd** — busca híbrida (BM25 + embeddings + reranking) sobre a wiki inteira; **primeira opção de busca sempre que disponível e indexada**, antes de `find_text.py`. Collection `secondbrain`, indexando `wiki/**/*.md`; o índice vive no cache global do usuário (`~/.cache/qmd`, fora do repo — nunca versionado, cada máquina reindexa a própria cópia). Antes de usar, confirme disponibilidade com `qmd status`; se o comando não existir ou a collection `secondbrain` não aparecer, caia para `scripts/find_text.py` sem perguntar ao Usuário. `qmd query "tema"` ganha de grep literal justamente na busca conceitual — "o que já escrevi sobre X", ecos entre um essay de filosofia e um de engenharia que usam vocabulário diferente para a mesma ideia; para achar um termo ou wikilink exato, `find_text.py` continua mais direto e não depende de índice atualizado. A indexação não é automática: `/organize` e `/stats` rodam `qmd update && qmd embed` no início de toda auditoria; fora desses fluxos, ofereça o mesmo comando depois de uma sessão com edição pesada de conteúdo, especialmente antes de `/status update` fechar a sessão.
- **scripts/find_text.py** — busca com trecho (grep -n com contexto) escopada às pastas da wiki, sem dependência externa — sempre funciona, mesmo sem qmd instalado ou indexado; é o fallback de `qmd` e a primeira opção quando ele não está disponível. Veja `README.md` (seção Estrutura de pastas → `scripts/`) para os outros scripts auxiliares (`build_index.py`, `build_references.py`, `check_references.py`, `check_dedupe.py`, `check_title.py`, `find_backlinks.py`).
