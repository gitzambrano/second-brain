...
8. Adicione wikilinks entre todas as páginas relacionadas, na seção `## Conexões`. Todo wikilink usa `[[slug-do-arquivo|Título Visível]]`, e link de seção usa `[[#Texto Do Heading]]` — as únicas formas que o Obsidian resolve (ver `## Regra de links` em `conventions/SKILL.md`).
