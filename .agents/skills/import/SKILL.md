...
10. Como o passo 5 acrescentou `## Referências`, escreva cada entrada no padrão de `## Formato de "## Referências" — padrão AIAA` em `conventions/SKILL.md` (`[N] Autor, *Título* [link](url), Container, Ano.`), confira com `python scripts/linkify_check.py --file <slug>`, e rode `python scripts/references_index.py` para regenerar `wiki/references.json`/`.md`. A fonte original quase nunca chega nesse formato: converter a bibliografia dela é parte do trabalho de ingestão, e não altera o texto do essay, que continua intacto.
