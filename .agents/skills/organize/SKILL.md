...
description: >
  ...
  conexões via scripts/build_graph.py. Use quando o Usuário disser "organiza
...
2. **Órfãos**: rode `python scripts/find_backlinks.py --orphans` em vez de recalcular do zero lendo todo `wiki/essays/*.md` manualmente. Para cada concept/entity sem essay que o referencie, decida com o Usuário se cria um essay novo, se anexa a um existente, ou se a página órfã deve ser removida por não servir a nada.
...
4. **`wiki/index.md`/`wiki/index.json`**: confirme que foram regenerados via `python scripts/build_index.py` (passo 1) — artefatos gerados, nunca editados à mão (ver `## Formato do índice` em `conventions/SKILL.md`). Se algum essay novo/editado/removido não estiver refletido, rode `build_index.py` de novo.
   ...
   **Cobertura do índice — o que falta em cada entrada.** Rode `python scripts/check_format.py --json` e separe os códigos de resumo:
...
13. **Links de toda a wiki — rode e corrija**: rode `python scripts/check_wiki.py` (cobre wikilinks mortos, órfãos, consistência de `index.md`, formatação mecânica, sources/manifesto nos dois sentidos, plano, insights — tudo dos passos acima, em um único relatório). Aplique `python scripts/fix_lint.py` para os achados mecânicos e inequívocos (espaçamento de heading, dois-pontos em display text de wikilink) — sem perguntar. Para cada **wikilink morto** reportado:
...
14. **Formato das `## Referências`**: rode `python scripts/check_references.py` sobre o corpus. Se aparecer `REFERENCIA_FORMATO_INVALIDO` em vários essays (sinal de bibliografia ainda no formato antigo, em bullet), sugira ao Usuário rodar `python scripts/check_references.py --fix-format` em lote — mas **não rode sozinho**: reformatar bibliografia é conteúdo, e o dono disso é `/linkify`. Os achados `REFERENCIA_SEM_LINK` e `LINK_NOT_IN_REFERENCIAS` que sobrarem entram no bucket "Atenção" do resumo final, porque pedem busca de fonte, não correção mecânica.
15. **Grafo**: rode `python scripts/build_graph.py` para gerar `output/graph/graph.html` (visualização interativa) e `output/graph/graph.md` (versão Mermaid). Ofereça abrir o HTML — é a forma mais rápida de ver clusters isolados, hubs, e partes da wiki que nunca se conectam entre si (ex: ensaios de filosofia que nunca linkam com os de engenharia).
16. **Exportação** (opcional, só se o Usuário quiser uma checagem completa): rode `python scripts/export_essay_pdf.py --all` e `python scripts/export_essay_html.py --all`, confirme que PDFs têm hyperlinks clicáveis, imagens resolvem, `## Conexões` não aparece no PDF e `## Referências`/`## Sumário` aparecem.
