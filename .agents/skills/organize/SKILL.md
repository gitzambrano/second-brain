...
1. Rode `python scripts/stats.py` para ver o estado atual — números de essays/tags/órfãos/sources sem manifesto.
...
9. **Links de toda a wiki — rode e corrija**: rode `python scripts/lint_all.py` (cobre wikilinks mortos em toda a wiki, órfãos, consistência de `index.md`, formatação mecânica). Aplique `python scripts/auto_fix_lint.py` para os achados mecânicos e inequívocos (espaçamento de heading, dois-pontos em display text de wikilink) — sem perguntar. Para cada **wikilink morto** reportado:
...
10. **Exportação** (opcional, só se o usuário quiser uma checagem completa): rode `python scripts/export_essay.py --all` e `python scripts/export_essay_html.py --all`, confirme que PDFs têm hyperlinks clicáveis, imagens resolvem, `## Conexões` não aparece no PDF e `## Referências`/`## Sumário` aparecem.
